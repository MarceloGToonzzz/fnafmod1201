...
public class DisplayShelfCleanupCommand {
	@SubscribeEvent
	public static void registerCommand(RegisterCommandsEvent event) {
		event.getDispatcher().register(Commands.literal("fnafmod")

				.then(Commands.literal("cleanup").then(Commands.argument("radius", DoubleArgumentType.doubleArg()).executes(arguments -> {
					...
					return 0;
				})))).then(Commands.literal("power").then(Commands.argument("PowerBlock", BlockPosArgument.blockPos()).then(Commands.argument("level", DoubleArgumentType.doubleArg(0, 100)).executes(arguments -> {
					Level world = arguments.getSource().getUnsidedLevel();
					double x = arguments.getSource().getPosition().x();
					double y = arguments.getSource().getPosition().y();
					double z = arguments.getSource().getPosition().z();
