...
public class UnwitheredFoxyEntity extends PathfinderMob implements GeoEntity {
	public static final EntityDataAccessor<Boolean> SHOOT = SynchedEntityData.defineId(UnwitheredFoxyEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> ANIMATION = SynchedEntityData.defineId(UnwitheredFoxyEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<String> TEXTURE = SynchedEntityData.defineId(UnwitheredFoxyEntity.class, EntityDataSerializers.STRING);
	...
	public UnwitheredFoxyEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(FnafModModEntities.UNWITHERED_FOXY.get(), world);
	}

	public UnwitheredFoxyEntity(EntityType<UnwitheredFoxyEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		setMaxUpStep(0.6f);
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "unwithered_foxy");
	}
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.unwitheredfoxy.walk"));
			}
			return event.setAndContinue(RawAnimation.begin().thenLoop("animation.unwitheredfoxy.showtime"));
		}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(UnwitheredFoxyEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
