...
public class PhantomFoxyEntity extends Monster implements GeoEntity {
	...
	public PhantomFoxyEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(FnafModModEntities.PHANTOM_FOXY.get(), world);
	}

	public PhantomFoxyEntity(EntityType<PhantomFoxyEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		setMaxUpStep(0.6f);
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(PhantomFoxyEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
