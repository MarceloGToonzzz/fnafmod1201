...
public class WitheredChicaEntity extends Monster implements GeoEntity {
	public static final EntityDataAccessor<Boolean> SHOOT = SynchedEntityData.defineId(WitheredChicaEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> ANIMATION = SynchedEntityData.defineId(WitheredChicaEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<String> TEXTURE = SynchedEntityData.defineId(WitheredChicaEntity.class, EntityDataSerializers.STRING);
	...
	public WitheredChicaEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(FnafModModEntities.WITHERED_CHICA.get(), world);
	}

	public WitheredChicaEntity(EntityType<WitheredChicaEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		setMaxUpStep(0.6f);
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.witheredchica.walk"));
			}
			return event.setAndContinue(RawAnimation.begin().thenLoop("animation.witheredchica.idle"));
		}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(WitheredChicaEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
