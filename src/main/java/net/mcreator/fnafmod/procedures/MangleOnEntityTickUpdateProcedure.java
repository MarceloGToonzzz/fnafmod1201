	...
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (world instanceof Level _lvl10 && _lvl10.isDay()) {
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = FnafModModEntities.MANGLE_SITTING.get().spawn(_level,
		...
		if ((world.getBlockState(BlockPos.containing(x, entity.getY() - 1, z))).getBlock() == FnafModModBlocks.VENT.get()) {
			...
			((Entity) world.getEntitiesOfClass(CrawlingMangleEntity.class, AABB.ofSize(new Vec3(x, y, z), 2, 2, 2), e -> true).stream().sorted(new Object() {
