	...
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		if (Random == 1) {
			...
			if (Random2 == 69) {
				...
				if (world instanceof ServerLevel _serverworld) {
					StructureTemplate template = _serverworld.getStructureManager().getOrCreate(new ResourceLocation("fnaf_mod", "thisisnothingcompletely"));
					if (template != null) {
						template.placeInWorld(_serverworld, BlockPos.containing(x, y + 69, z), BlockPos.containing(x, y + 69, z), new StructurePlaceSettings().setRotation(Rotation.NONE).setMirror(Mirror.NONE).setIgnoreEntities(false),
