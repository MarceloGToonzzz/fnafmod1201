		...
		if (sourceentity instanceof TheCrooblerEntity) {
			if (!sourceentity.getPersistentData().getBoolean("reload")) {
				{
					...
					if (!projectileLevel.isClientSide()) {
						...
						}.getArrow(projectileLevel, sourceentity, 5, 0);
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("fnaf_mod:ak-sound")), SoundSource.HOSTILE, 1, 1);
					} else {
			...
			if (entity.getPersistentData().getDouble("shootamount") == 30) {
				...
				if (sourceentity.getPersistentData().getBoolean("reload")) {
					if (world instanceof Level _level) {
						if (!_level.isClientSide()) {
							_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("fnaf_mod:akreload")), SoundSource.HOSTILE, 1, 1);
						} else {
							_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("fnaf_mod:akreload")), SoundSource.HOSTILE, 1, 1, false);
						}
					}
