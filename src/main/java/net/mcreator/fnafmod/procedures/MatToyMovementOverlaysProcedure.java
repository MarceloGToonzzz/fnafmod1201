...
public class MatToyMovementOverlaysProcedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			MatToyMovementOverlaysProcedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				MatToyMovementOverlaysProcedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	private static void execute(@Nullable Event event) {
		if (target(3)) {
			if (Minecraft.getInstance().player != null) {
				...
				if (entity instanceof Player _plr0 && _plr0.containerMenu instanceof ToyMovementMenu) {
					if (!(((new BiFunction<LevelAccessor, String, Entity>() {
					...
					}).apply(world, (entity.getPersistentData().getString("guiTarget")))) == null)) {
						RenderSystem.defaultBlendFunc();
						if (((new BiFunction<LevelAccessor, String, Entity>() {
							@Override
							public Entity apply(LevelAccessor levelAccessor, String uuid) {
						...
						}).apply(world, (entity.getPersistentData().getString("guiTarget")))).getPersistentData().getBoolean("hasToyMovementToggle")) {
							RenderSystem.setShaderTexture(0, new ResourceLocation(("fnaf_mod" + ":textures/" + "screens/toy_movement_button_on" + ".png")));
						} else {
