		...
		if (!(FnafModModVariables.WorldVariables.get(world).Connections).isEmpty()) {
			...
			while (connect.contains("x")) {
				...
				if (x == xPosition && y == yPosition && z == zPosition || x == xPosition2 && y == yPosition2 && z == zPosition2) {
					newConnection = FnafModModVariables.WorldVariables.get(world).Connections;
					while (newConnection.contains("x")) {
						if (index != index2) {
							newConnection = newConnection + "" + newConnection.substring((int) newConnection.indexOf("x"), (int) newConnection.indexOf("f") + "f".length());
						}
						index2 = index2 + 1;
					}
